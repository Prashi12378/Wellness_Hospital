...
import { Loader2, Printer, ArrowLeft, Download, CheckCircle2, Pencil, X, Save } from "lucide-react";
...
export default function LabReportPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const seedFromRequest = (data: any) => {
        setPatientName(data.patientName || "");
        setDoctorName(data.requestedByName || "");
        setConsultantName(data.consultantName || "");
        setCollectedAt(toLocalInput(data.createdAt));
        setReceivedAt(toLocalInput(data.createdAt));
        setReportedAt(toLocalInput(data.updatedAt));
    };
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            // Persist patient name if changed
            if (patientName.trim() && patientName.trim() !== request.patientName) {
                await updatePatientNameOnReport(id, patientName.trim());
            }
            // Persist doctor/ref name if changed
            if (doctorName.trim() !== (request.requestedByName || "")) {
                await updateDoctorNameOnReport(id, doctorName.trim());
            }
            // Update local state to reflect saved values
            setRequest((prev: any) => ({
                ...prev,
                patientName: patientName.trim() || prev.patientName,
                requestedByName: doctorName.trim(),
    ...
    const handleCancel = () => {
        if (request) seedFromRequest(request);
        setEditing(false);
    };
            ...
            <div id="printable-report" className="bg-white relative p-8 md:p-12 rounded-[48px] border border-slate-100 shadow-sm print:shadow-none print:border-0 print:p-0 print:rounded-none min-h-[1000px] flex flex-col font-sans mb-10 print:bg-transparent">
                ...
                <div className="grid grid-cols-2 gap-6 mb-8 border border-slate-100 rounded-xl p-6 bg-slate-50/10">
                    <div className="space-y-1.5 border-r border-slate-100 pr-6">
                        ...
                        {/* Patient Name */}
                        <div className="flex text-[10px] leading-tight items-baseline gap-1">
                            ...
                            {editing ? (
                                <>
                                    <input
                                        ...
                                        className="print:hidden font-black text-slate-900 text-[10px] border border-slate-300 rounded px-2 py-0.5 bg-white focus:outline-none focus:ring-1 focus:ring-slate-400 uppercase w-44"
                                    />
                            ...
                            {editing ? (
                                <>
                                    <input
                                        ...
                                        className="print:hidden font-black text-blue-800 text-[10px] border border-blue-300 rounded px-2 py-0.5 bg-blue-50 focus:outline-none focus:ring-1 focus:ring-blue-400 uppercase w-44 italic"
                                    />
                                    ...
                                    return (
                                        <div key={idx}>
                                            ...
                                            <div className="grid grid-cols-[2fr_100px_150px_100px] gap-16 items-start py-1">
                                                <span className={cn("text-[10px] font-bold text-slate-700 leading-tight", isGrouped && "pl-6")}>{mainName}</span>
                            ...
                            {editing ? (
                                <>
                                    <input
                                        ...
                                        className="print:hidden text-[11px] font-black text-slate-900 uppercase tracking-tight border border-slate-300 rounded px-2 py-0.5 bg-white focus:outline-none focus:ring-1 focus:ring-slate-400 text-right w-52 mb-1"
                                    />
                                    <span className="hidden print:block text-[11px] font-black text-slate-900 uppercase tracking-tight leading-none mb-1">{consultantName || request.consultantName || 'Dr. Somashekar K.'}</span>
                                </>
                            ) : (
                                <p className="text-[11px] font-black text-slate-900 uppercase tracking-tight leading-none mb-1">
                                    {consultantName || request.consultantName || 'Dr. Somashekar K.'}
                                </p>
                            )}
